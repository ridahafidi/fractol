...
/*   Updated: 2025/02/26 21:38:11 by rhafidi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	initiate(t_mlx_data *mlx, t_data *img)
{
	...
	if (!mlx->mlx_connection)
		exit(1);
	mlx->mlx_win = mlx_new_window(mlx->mlx_connection, WIDTH, HEIGHT,
			"Fractol");
	img->img = mlx_new_image(mlx->mlx_connection, WIDTH, HEIGHT);
	if (!img->img || !mlx->mlx_win)
	{
...
{
	initiate(mlx, img);
	display_mandelbortset(mlx, img, mlx->img->current_color);
...
{
	initiate(mlx, img);
	img->c.real = ft_atoi(c_real, 0, 0, 0);
	img->c.i = ft_atoi(c_img, 0, 0, 0);
	display_julia(mlx, img, mlx->img->current_color);
...
void	my_set(t_mlx_data *mlx, t_data *img)
{
	initiate(mlx, img);
	display_my_set(mlx, img, mlx->img->current_color);
	mlx_key_hook(mlx->mlx_win, &handle_keys_for_my_set, mlx);
	mlx_mouse_hook(mlx->mlx_win, &handle_mouse_for_my_set, mlx);
	mlx_hook(mlx->mlx_win, 17, 0, &handle_destroy, mlx);
	mlx_expose_hook(mlx->mlx_win, &handle_expose_for_my_set, mlx);
	mlx_loop(mlx->mlx_connection);
}
	...
	if (ac <= 4)
	{
		if (ac == 2)
		{
			if (!ft_strncmp(av[1], "mandelbrot", 11))
				mandelbrot(&mlx, &img);
			if (!ft_strncmp(av[1], "mandelbrot twin", 16))
				my_set(&mlx, &img);
		}
		else if (ac == 4)
		{
